...
jboolean
Java_com_anysdk_sample_wrapper_nativeIsSupportFunction(JNIEnv*  env, jobject thiz, jstring code)
{
	std::string strClassName = PluginJniHelper::jstring2string(code);
	return (jboolean)PluginChannel::getInstance()->isSupportFunction(strClassName);
}
...
Java_com_anysdk_sample_wrapper_nativeExcuteFunction(JNIEnv*  env, jobject thiz, jstring code)
{
	...
	if (PluginChannel::getInstance()->isSupportFunction(strClassName)) {
